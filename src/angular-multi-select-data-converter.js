	...
	DataConverter.prototype.check_prerequisites = function (data) {
		...
		function process_items (items) {
			...
			for (var i = 0; i < items.length; i++) {
				...
				if (!item.hasOwnProperty(ctx.ID_PROPERTY) || ids.indexOf(item[ctx.ID_PROPERTY]) !== -1) item[ctx.ID_PROPERTY] = gen_id();
				ids.push(item[ctx.ID_PROPERTY]);

				// Check for open field.
				// If open field doesn't exist or is not "true", set to false
				if (!item.hasOwnProperty(ctx.OPEN_PROPERTY) || item[ctx.OPEN_PROPERTY] !== true) item[ctx.OPEN_PROPERTY] = false;

				// Check for children field.
				// If not an array or empty array, remove it.
				if (item.hasOwnProperty(ctx.CHILDREN_PROPERTY) && !Array.isArray(item[ctx.CHILDREN_PROPERTY])) delete item[ctx.CHILDREN_PROPERTY];
				if (item.hasOwnProperty(ctx.CHILDREN_PROPERTY) && item[ctx.CHILDREN_PROPERTY].length === 0) delete item[ctx.CHILDREN_PROPERTY];

				// If children field is present, remove "checked" field.
				// If checked field is present, but value is not boolean or 1,
				// set to false.
				if (item.hasOwnProperty(ctx.CHILDREN_PROPERTY)) delete item[ctx.CHECKED_PROPERTY];
				if (item.hasOwnProperty(ctx.CHECKED_PROPERTY) && (item[ctx.CHECKED_PROPERTY] !== true && item[ctx.CHECKED_PROPERTY] !== 1)) item[ctx.CHECKED_PROPERTY] = false;
				if (!item.hasOwnProperty(ctx.CHILDREN_PROPERTY) && !item.hasOwnProperty(ctx.CHECKED_PROPERTY)) item[ctx.CHECKED_PROPERTY] = false;

				if (item.hasOwnProperty(ctx.CHILDREN_PROPERTY)) process_items(item[ctx.CHILDREN_PROPERTY]);
			}
		}
	...
	DataConverter.prototype.to_internal = function (data) {
		...
		function process_items (items, level) {
			for (var i = 0; i < items.length; i++) {
				...
				if (item.hasOwnProperty(ctx.CHECKED_PROPERTY) && typeof(item[ctx.CHECKED_PROPERTY]) === 'boolean') {
					final_item[ctx.CHECKED_PROPERTY] = item[ctx.CHECKED_PROPERTY];
				} else {
					final_item[ctx.CHECKED_PROPERTY] = null;
				}
				...
				final_item.tree_visibility = true;

				final_data.push(final_item);

				if (item.hasOwnProperty(ctx.CHILDREN_PROPERTY)) process_items(item[ctx.CHILDREN_PROPERTY], level + 1);
			}
		}
		...
		for (i = 0; i < final_data.length; i++) {
			...
			for (j = i + 1; j < final_data.length; j++) {
				...
				if (child[ctx.CHECKED_PROPERTY] === true) {
					counter_checked++;
					item.children_leafs++;
				} else if (child[ctx.CHECKED_PROPERTY] === false) {
					counter_unchecked++;
					item.children_leafs++;
				} else if (child[ctx.CHECKED_PROPERTY] === null){
					counter_null++;
					item.children_nodes++;
				}
			...
			if (item.children_leafs === counter_checked) {
				item[ctx.CHECKED_PROPERTY] = 1; //all checked
			} else if (item.children_leafs === counter_unchecked) {
				item[ctx.CHECKED_PROPERTY] = -1; //all unchecked
			} else {
				item[ctx.CHECKED_PROPERTY] = 0; //mixed
			}
